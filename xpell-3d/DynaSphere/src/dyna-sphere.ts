...
export class DynaSphere extends X3DObject {
    ...
    constructor(data: any, defaults = {
        ...
        this._material = new THREE.PointsMaterial({ vertexColors: true, size: 0.1, transparent: true });
    ...
    async onFrame(frameNumber: number): Promise<void> {
        ...
        if (this._audio_analyzer) {
            ...
            for (let i = 0; i < this._points; i++) {
                ...
                const multiplier = (i < dataArray.length * 3) ? dataArray[i] / 256 * 2 : 0
                ...
                newPositions[i * 3] = x * (multiplier + (2 * this._radius))
                newPositions[i * 3 + 1] = y * (multiplier + (2 * this._radius))
                newPositions[i * 3 + 2] = z * (multiplier + (2 * this._radius))


                const clr = new THREE.Color(colors[i * 3], colors[i * 3 + 1], colors[i * 3 + 2]);
                // dimm the color based on the audio data
                clr.r = (1 + multiplier) * clr.r //* (1 - average / 256) + 0.2;
                clr.g = (1 + multiplier) * clr.g //* (1 - average / 256) + 0.2;
                clr.b = (1 + multiplier) * clr.b //* (1 - average / 256) + 0.2;
